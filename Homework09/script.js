...
function Slider({ wrapperId, itemsId, prevId, nextId, paginationId, slides }) {
	...
	this.itemsEl.addEventListener("mousemove", (e) => {
		...
		this.itemsEl.style.transform =  `translate(${-this.currentIndex * wrapperWidth}px, 0)`;
		let style = this.itemsEl.style.transform;
		let trfRegExp = /[-0-9.]+(?=px)/;
		let transform = +style.match(trfRegExp)[0];

			this.itemsEl.style.transform = `translate(${transform-diffX}px, 0)`;
...
//		this.itemsEl.style.transform = `translate(${transform-diffX}px, 0)`;
	...
	// == Homework - keyboard switching slides ==================
	document.addEventListener('keydown', (e) => {
		if(e.code == 'ArrowLeft') this.moveToIndex(this.currentIndex + 1);
		if(e.code == 'ArrowRight') this.moveToIndex(this.currentIndex - 1);
	})
